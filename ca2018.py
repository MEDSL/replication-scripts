...
df = pd.read_csv('2018-ca-precinct-autoadapted.zip', dtype = official_dtypes)
df = df.fillna("")
...
df['district'] = df.district.str.zfill(3).str.replace('000','',regex=False)

df['candidate'] = df['candidate'].replace('\.',"", regex=True)
df['candidate'] = df['candidate'].replace(['TOTAL REG','TOTAL VOTES'],
...
df.to_csv("2018-ca-precinct-general-updated.csv",quoting=csv.QUOTE_NONNUMERIC,index=False)