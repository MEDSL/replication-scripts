...
df = pd.read_csv('2018-ne-precinct-autoadapted.csv', dtype = official_dtypes)
df = df.fillna("")
...
df['candidate'] = df['candidate'].replace(['FOR','AGAINST'],['YES','NO'])
df['candidate'] = df['candidate'].str.replace('\.','',regex=True)
df['district'] = df['district'].str.zfill(3).replace('000','')
...
df.to_csv("2018-ne-precinct-general-updated.csv",quoting=csv.QUOTE_NONNUMERIC,index=False)