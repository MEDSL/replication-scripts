...
df = pd.read_csv('2018-de-precinct-autoadapted.csv', dtype = official_dtypes)
df = df.fillna("")
...
df['district'] = df['district'].str.zfill(3)
df.loc[df['candidate']=='PAUL AJOHNSTON', 'candidate'] = 'PAUL A JOHNSTON'
df['district'] = np.where((df['district'] == '000')&(df['office']!='US HOUSE'),"",df['district'])
df.loc[(df['party_detailed']=="")&(df['party_simplified']=='OTHER'),
    'party_simplified'] = ""
...
df.to_csv("2018-de-precinct-general-updated.csv",quoting=csv.QUOTE_NONNUMERIC,index=False)