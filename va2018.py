...
df = pd.read_csv('2018-va-precinct-autoadapted.csv', dtype = official_dtypes)
df = df.fillna("")

df['county_name'] = df['county_name'].str.replace('KING & QUEEN', 'KING AND QUEEN', regex=False)
...
#string pad
df['district'] = df['district'].str.zfill(3).replace(['00A','00B','00C','000'],['A','B','C',""])

df['date'] = '2018-11-06'
df['readme_check'] = 'FALSE'
mags = pd.read_csv('magnitudes.csv').fillna("")
df = df.merge(mags, on = ['office','jurisdiction_name','district'], how = 'left')
df['magnitude'] = df['magnitude'].fillna(1).astype(int)
...
df.loc[df['office']=='MEMBER BOARD OF SUPERVISORS AT LARGE', 'district'] = 'AT-LARGE'
df.loc[df['office']=='MEMBER BOARD OF SUPERVISORS AT LARGE', 'office'] = 'MEMBER BOARD OF SUPERVISORS'

#################################### specific ####################################

ammendments = pd.read_csv('va-18-ammendments.csv',dtype = official_dtypes)
...
df.to_csv("2018-va-precinct-general-updated.csv",quoting=csv.QUOTE_NONNUMERIC,index=False)